...
async def view_code(request: Request):
    ...
    if "/conversion" in url_path:
        return templates.TemplateResponse("view/code/conversion.html", {"request": request, "message": "용어변환"})
    elif "/completion" in url_path:
        return templates.TemplateResponse("view/code/completion.html", {"request": request, "message": "프로그램 코드 생성"})
    elif "/text2sql" in url_path:
        return templates.TemplateResponse("view/code/text2sql.html", {"request": request, "message": "SQL 생성"})
    elif "/chat" in url_path:
        return templates.TemplateResponse("view/code/chat.html", {"request": request, "message": "RAG 요청"})
    else :
